    ...
    def _create_account_move_line(self, session=None, move=None):
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == 'paid'):
            ...
            for line in order.lines:
                ...
                # New Change ( convert the income account to the one in the fiscal position )
                new_income_account = self.env['account.account'].browse(income_account)
                income_account = order.fiscal_position_id.map_account(new_income_account).id
                # End of Change
